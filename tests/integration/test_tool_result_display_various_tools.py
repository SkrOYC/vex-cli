...
def test_tool_result_display_with_bash_tool(none_tool_adapter):
    ...
    event = ToolResultEvent(
        tool_name="bash", tool_class=None, tool_call_id="bash_123", result=bash_result
    )
...
def test_tool_result_display_with_read_file_tool(none_tool_adapter):
    ...
    mock_result = MockToolResult(
        status="success", data={"content": "file content", "path": "/path/to/file.txt"}
    )
    event = ToolResultEvent(
        tool_name="read_file",
        tool_class=None,
...
def test_tool_call_display_with_various_arg_types(none_tool_adapter):
    ...
    complex_args = MockToolArgs(message="test message", count=42)
    event = ToolCallEvent(
...
def test_tool_result_display_with_none_result(none_tool_adapter):
    ...
    event = ToolResultEvent(
        tool_name="silent_tool", tool_class=None, tool_call_id="silent_123", result=None
    )
