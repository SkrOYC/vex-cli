...
def tool_config(
    ...
    """Create an StrReplaceToolConfig with ViewTrackerService."""
    return StrReplaceToolConfig(view_tracker=view_tracker, workdir=temp_dir)


@pytest.fixture
def tool_config_no_view_tracker(temp_dir: Path) -> StrReplaceToolConfig:
    """Create an StrReplaceToolConfig without ViewTrackerService."""
    return StrReplaceToolConfig(workdir=temp_dir)


@pytest.fixture
def tool_state() -> StrReplaceToolState:
    """Create a fresh StrReplaceToolState for each test."""
    return StrReplaceToolState()


@pytest.fixture
def tool(
    tool_config: StrReplaceToolConfig, tool_state: StrReplaceToolState
) -> StrReplaceTool:
    """Create an StrReplaceTool instance for testing."""
    return StrReplaceTool(config=tool_config, state=tool_state)


@pytest.fixture
def tool_no_view_tracker(
    tool_config_no_view_tracker: StrReplaceToolConfig, tool_state: StrReplaceToolState
) -> StrReplaceTool:
    """Create an StrReplaceTool instance without ViewTrackerService for testing."""
    return StrReplaceTool(config=tool_config_no_view_tracker, state=tool_state)


# =============================================================================
# StrReplaceArgs Tests
# =============================================================================


class TestEditFileArgs:
    """Tests for StrReplaceArgs Pydantic model."""

    def test_creation_with_path_and_strings(self) -> None:
        """Test StrReplaceArgs can be created with path, old_str, and new_str."""
        args = StrReplaceArgs(path="/test/file.txt", old_str="hello", new_str="world")
    ...
    def test_creation_with_empty_strings(self) -> None:
        """Test StrReplaceArgs accepts empty strings."""
        args = StrReplaceArgs(path="/test/file.txt", old_str="", new_str="")
        assert args.old_str == ""
        assert args.new_str == ""

    def test_creation_with_whitespace(self) -> None:
        """Test StrReplaceArgs handles whitespace correctly."""
        args = StrReplaceArgs(
    ...
    def test_model_validation(self) -> None:
        """Test StrReplaceArgs validates types correctly."""
        with pytest.raises(ValidationError):
            StrReplaceArgs(path=123, old_str="text", new_str="text")  # type: ignore


# =============================================================================
# StrReplaceResult Tests
# =============================================================================


class TestEditFileResult:
    """Tests for StrReplaceResult Pydantic model."""

    def test_creation_with_output(self) -> None:
        """Test StrReplaceResult can be created with output."""
        result = StrReplaceResult(output="File modified successfully.")
...
class TestStrReplaceSuccess:
    ...
    async def test_replace_exactly_one_occurrence(
        self, tool: StrReplaceTool, temp_dir: Path
    ) -> None:
        ...
        result = await tool.run(
            StrReplaceArgs(path=str(file_path), old_str="world", new_str="bun")
        )
    ...
    async def test_replace_with_empty_new_str(
        ...
        result = await tool.run(
            StrReplaceArgs(path=str(file_path), old_str=" world", new_str="")
        )
    ...
    async def test_replace_saves_to_edit_history(
        ...
        await tool.run(
            StrReplaceArgs(path=str(file_path), old_str="original", new_str="modified")
        )
    ...
    async def test_replace_updates_view_timestamp(
        self, tool: StrReplaceTool, view_tracker: ViewTrackerService, temp_dir: Path
    ) -> None:
    ...
    async def test_replace_without_view_tracker(
        self, tool_no_view_tracker: StrReplaceTool, temp_dir: Path
    ) -> None:
        ...
        result = await tool_no_view_tracker.run(
            StrReplaceArgs(path=str(file_path), old_str="hello", new_str="world")
        )
...
class TestStrReplaceTextNotFound:
    ...
    async def test_replace_fails_when_old_str_not_found(
        ...
        with pytest.raises(FileSystemError) as exc_info:
            await tool.run(
                StrReplaceArgs(
                    path=str(file_path), old_str="nonexistent", new_str="text"
                )
            )
    ...
    async def test_error_message_includes_helpful_suggestions(
        ...
        with pytest.raises(FileSystemError) as exc_info:
            await tool.run(
                StrReplaceArgs(path=str(file_path), old_str="goodbye", new_str="hello")
            )
    ...
    async def test_replace_fails_on_empty_file(
        ...
        with pytest.raises(FileSystemError) as exc_info:
            await tool.run(
                StrReplaceArgs(path=str(file_path), old_str="anything", new_str="text")
            )
    ...
    async def test_replace_fails_with_empty_old_str(
        ...
        with pytest.raises(FileSystemError) as exc_info:
            await tool.run(
                StrReplaceArgs(path=str(file_path), old_str="", new_str="replacement")
            )
...
class TestStrReplaceMultipleMatches:
    ...
    async def test_replace_fails_when_multiple_occurrences(
        ...
        with pytest.raises(FileSystemError) as exc_info:
            await tool.run(
                StrReplaceArgs(path=str(file_path), old_str="repeat", new_str="once")
            )
    ...
    async def test_error_message_includes_count_and_suggestions(
        ...
        with pytest.raises(FileSystemError) as exc_info:
            await tool.run(
                StrReplaceArgs(path=str(file_path), old_str="foo", new_str="bar")
            )
...
class TestFileNotFound:
    ...
    async def test_replace_fails_on_nonexistent_file(
        ...
        with pytest.raises(FileSystemError) as exc_info:
            await tool.run(
                StrReplaceArgs(path=str(file_path), old_str="old", new_str="new")
            )
...
class TestPathResolution:
    ...
    async def test_relative_path_resolved_against_workdir(
        ...
        result = await tool.run(
            StrReplaceArgs(path="relative.txt", old_str="original", new_str="modified")
        )
...
class TestUTF8Encoding:
    ...
    async def test_utf8_encoding_handles_special_characters(
        ...
        result = await tool.run(
            StrReplaceArgs(path=str(file_path), old_str="世界", new_str="宇宙")
        )
...
class TestToolConfiguration:
    """Tests for StrReplaceTool configuration."""

    def test_tool_name_is_str_replace(self) -> None:
        """Test tool has correct name."""
        assert StrReplaceTool.name == "str_replace"

    def test_tool_has_description(self) -> None:
        """Test tool has description."""
        assert len(StrReplaceTool.description) > 0

    def test_tool_uses_str_replace_args_schema(self) -> None:
        """Test tool uses StrReplaceArgs as schema."""
        assert StrReplaceTool.args_schema == StrReplaceArgs
...
class TestEditHistory:
    ...
    async def test_multiple_edits_save_to_history(
        ...
        await tool.run(
            StrReplaceArgs(
                path=str(file_path), old_str="version 1", new_str="version 2"
            )
        )
        # Second edit
        await tool.run(
            StrReplaceArgs(
                path=str(file_path), old_str="version 2", new_str="version 3"
            )
        )
...
class TestEdgeCases:
    ...
    async def test_replace_with_exact_file_content(
        ...
        result = await tool.run(
            StrReplaceArgs(path=str(file_path), old_str=content, new_str="new content")
        )
    ...
    async def test_replace_with_special_regex_chars(
        ...
        result = await tool.run(
            StrReplaceArgs(path=str(file_path), old_str="(1)", new_str="[2]")
        )
